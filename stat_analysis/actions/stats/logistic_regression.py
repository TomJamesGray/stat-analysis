    ...
    def run(self,validate=True,quiet=False,use_cached=False,**kwargs):
        ...
        for val in y:
            if val not in y_options and len(y_options) == 2:
                ...
                # This value doesn't already appear in the y options so add it to the list of y options
        ...
        if not quiet:
            # Get the coefficient and constant terms generated by the logisitc regression model
            ...
            # Output the coeff, constant term and percentage accuracy in a table
            ...
            if vals["inverse_logit"] != None:
                ...
                for val in vals["inverse_logit"]:
                    if not (0 < val <1):
                        ...
                        # Skip this value
                        continue
                    # Find the x value that corresponds to the probability
                    predicted_x.append(str(np.round(-1/x_coeff * (np.log(1/val -1)+const),3)))
                    used_p_vals.append(val)

                if len(used_p_vals) != 0:
                    # Output the probabilites and x values in a table if some have been generated
                    self.result_output.add_widget(BorderedTable(
            ...
            if vals["make_graph"]:
                ...
                # Create graph figure and subplots
                fig = plt.figure()
                axis = plt.subplot(111)
                # Set y limits to 0 and 1 as y values are probabilities so range from 0 to 1
                axis.set_ylim([0,1])
                # Scatter y and x data
                ...
                axis.legend()
                # Make the path for the graph image
